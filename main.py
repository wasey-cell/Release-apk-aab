...
from kivymd.uix.button import MDRaisedButton,MDFlatButton, MDRoundFlatButton, MDRectangleFlatButton,MDFillRoundFlatIconButton,MDRoundFlatIconButton,MDRectangleFlatIconButton ,MDIconButton
...
Screen:
    FloatLayout:
        ...
        MDBoxLayout:
            ...
            MDLabel:
                text: '[font=Oswald.ttf]X0X Gam€[/font]'
                markup:True
                bold:True
                halign: 'center'
                font_size: '50sp'
...
class MainScreen(MDScreen):
    def __init__(self, **kwargs):
        ...
        title_label = MDLabel(
            text="[font=WinkyRough.ttf][size=60] X[color=#FF6B6B]O[/color]X [color=#4ECDC4]G[/color][color=#45B6D1]A[/color][color=#96CEB4]M[/color][color=#FFEAA7]E[/color] [/size][/font]",
            halign="center",
            markup=True,
            theme_text_color="Custom",
            text_color=(1, 1, 1, 1)
        ...
        subtitle = MDLabel(
            text="[font=ComicRelief.ttf][i]Choose your battle mode![/i][/font]",
            halign="center",
            font_style="H4",
            markup=True,
            theme_text_color="Custom",
        ...
        # Create float layout for buttons
        f_lay = FloatLayout()
        
        # Enhanced Single Player Button
        btn1 = MDCard(
            elevation=12,
            radius=[25, 25, 25, 25],
            size_hint=(None, None),
            ...
            md_bg_color=[0.2, 0.7, 1, 0.9],
            ripple_behavior=True
        )
        
        btn1_layout = MDBoxLayout(
            orientation="horizontal",
            spacing="20dp",
            padding="20dp"
        )
        
        btn1_icon = MDIconButton(
            icon="robot-outline",
            icon_size="50sp",
            theme_text_color="Custom",
            text_color=(1, 1, 1, 1),
        ...
        btn1_text = MDLabel(
            text="[font=Poppins.ttf]VS AI\n[size=20dp]Challenge the Bot![/size][/font]",
            markup=True,
            halign="left",
            valign="middle",
            theme_text_color="Custom",
            text_color=(1, 1, 1, 1),
            font_style="H5",
            text_size=("240dp", None)
        )
        
        btn1_layout.add_widget(btn1_icon)
        btn1_layout.add_widget(btn1_text)
        btn1.add_widget(btn1_layout)
        btn1.bind(on_press=lambda x: self.button_animation(btn1, lambda: self.set_game(True)))
        btn1_icon.bind(on_press=lambda x: self.set_game(True))
        # Enhanced Two Player Button
        btn2 = MDCard(
            elevation=12,
            radius=[25, 25, 25, 25],
            size_hint=(None, None),
            ...
            md_bg_color=[1, 0.3, 0.5, 0.9],
            ripple_behavior=True
        )
        
        btn2_layout = MDBoxLayout(
            orientation="horizontal",
            spacing="20dp",
            padding="20dp"
        )
        
        btn2_icon = MDIconButton(
        ...
        btn2_text = MDLabel(
            text="[font=Poppins.ttf]2 PLAYERS\n[size=20dp]Battle Your Friend![/size][/font]",
            markup=True,
            halign="left",
            valign="middle",
            theme_text_color="Custom",
        ...
        btn2_layout.add_widget(btn2_icon)
        btn2_layout.add_widget(btn2_text)
        btn2.add_widget(btn2_layout)
        btn2.bind(on_press=lambda x: self.button_animation(btn2, lambda: self.set_game(False)))
        btn2_icon.bind(on_press=lambda x: self.set_game(False))
        ...
        f_lay.add_widget(decoration1)
        f_lay.add_widget(decoration2)
        f_lay.add_widget(decoration3)
        f_lay.add_widget(decoration4)
        
        main_layout.add_widget(f_lay)
        
        # Add version info at bottom
        version_label = MDLabel(
            text="v1.0 - Enhanced Edition",
            halign="center",
            font_style="Caption",
    ...
    def create_animated_background(self):
        ...
        for i in range(15):
            element = MDIcon(
                icon=random.choice(["close-circle-outline", "checkbox-blank-circle-outline"]),
                font_size=f"{random.randint(20, 40)}sp",
                pos_hint={
                    "center_x": random.random(),
                    "center_y": random.random()
                },
                theme_text_color="Custom",
                text_color=[*self.theme_cls.primary_color[:3], 0.1]
            )
    ...
    def animate_element(self, element):
        ...
        anim.bind(on_complete=lambda anim, widget: self.animate_element(widget))
        anim.start(element)
    
    def button_animation(self, button, callback):
        from kivy.animation import Animation
        from kivy.metrics import dp
        ...
        # Chain animations
        anim1.bind(on_complete=lambda anim, widget: anim2.start(widget))
        anim2.bind(on_complete=lambda anim, widget: callback())
...
class ActApp(MDScreen):  
    ...
    def on_back_button(self, window, key, *args):
        if key == 27:
            if hasattr(self, 'toss_dialog') and self.toss_dialog:
    ...
    def toss(self):
        ...
        self.tos_label=MDLabel(
            text=f"{self.pp1}'s call",
            size_hint=(None, None),
            pos_hint={'center_x':0.5},
            size=('350dp', '120dp'),
            halign="center",
        ...
        content.add_widget(self.tos_label
        
    )
    
        content.add_widget(MDRoundFlatIconButton(text="Heads", icon='brain', size_hint=(None,None), size=("50dp","15dp"), pos_hint={'center_x':0.5}, on_release=lambda x: self.toss_win('head')))
        content.add_widget(MDRoundFlatIconButton(text="Tails", icon='paw', pos_hint={'center_x':0.5}, size_hint=(None,None), size=("50dp","15dp"), on_release=lambda x: self.toss_win('tail')))
    
        self.toss_dialog = MDDialog(title="Toss", type="custom", size_hint=(None,None), size=('300dp','400dp'), auto_dismiss=False, content_cls=content)
    ...
    # Add this check to prevent moves during AI turn
        if self.bot and self.turn == 'p2':
            return
...
class set_diff(MDScreen):
    def __init__(self, **kwargs):
        ...
        title_label = MDLabel(
            text="[font=WinkyRough.ttf][size=50] CHOOSE [color=#FF6B6B]YOUR[/color] [color=#4ECDC4]DIFFICULTY[/color] [/size][/font]",
            halign="center",
            markup=True,
            theme_text_color="Custom",
            text_color=(1, 1, 1, 1)
        ...
        # Subtitle with animation
        subtitle = MDLabel(
            text="[font=ComicRelief.ttf][i]Test your skills against the AI![/i][/font]",
            halign="center",
            font_style="H6",
            markup=True,
            theme_text_color="Custom",
        ...
        btn1_text = MDLabel(
            text="[font=Poppins.ttf]EASY\n[size=20dp]Beginner Friendly![/size][/font]",
            markup=True,
            halign="left",
            valign="middle",
            theme_text_color="Custom",
        ...
        btn1_layout.add_widget(btn1_text)
        btn1.add_widget(btn1_layout)
        btn1.bind(on_press=lambda x: self.button_animation(btn1, lambda: self.setdif("easy")))
        btn1_icon.bind(on_press=lambda x: self.setdif("easy"))
        
        # Enhanced Medium Button
        btn2 = MDCard(
            elevation=12,
            radius=[25, 25, 25, 25],
            size_hint=(None, None),
            ...
            md_bg_color=[1, 0.7, 0.2, 0.9],  # Orange for medium
            ripple_behavior=True
        )
        ...
        btn2_text = MDLabel(
            text="[font=Poppins.ttf]MEDIUM\n[size=20dp]Balanced Challenge![/size][/font]",
            markup=True,
            halign="left",
            valign="middle",
            theme_text_color="Custom",
        ...
        btn2_layout.add_widget(btn2_text)
        btn2.add_widget(btn2_layout)
        btn2.bind(on_press=lambda x: self.button_animation(btn2, lambda: self.setdif("medium")))
        btn2_icon.bind(on_press=lambda x: self.setdif("medium"))
        
        # Enhanced Hard Button
        btn3 = MDCard(
            elevation=12,
            radius=[25, 25, 25, 25],
            size_hint=(None, None),
            ...
            md_bg_color=[0.9, 0.3, 0.3, 0.9],  # Red for hard
            ripple_behavior=True
        )
        
        btn3_layout = MDBoxLayout(
            orientation="horizontal",
            spacing="20dp",
            padding="20dp"
        )
        
        btn3_icon = MDIconButton(
        ...
        btn3_text = MDLabel(
            text="[font=Poppins.ttf]HARD\n[size=20dp]Expert Mode![/size][/font]",
            markup=True,
            halign="left",
            valign="middle",
            theme_text_color="Custom",
        ...
        btn3_layout.add_widget(btn3_icon)
        btn3_layout.add_widget(btn3_text)
        btn3.add_widget(btn3_layout)
        btn3.bind(on_press=lambda x: self.button_animation(btn3, lambda: self.setdif("hard")))
        btn3_icon.bind(on_press=lambda x: self.setdif("hard"))
        ...
        # Add difficulty description at bottom
        description_label = MDLabel(
            text="Choose wisely - each level brings new challenges!",
            halign="center",
            font_style="Caption",
            theme_text_color="Secondary",
            size_hint_y=None,
    ...
    def create_animated_background(self):
        ...
        for i in range(16):
            element = MDIcon(
                icon=random.choice(["close-circle-outline", "checkbox-blank-circle-outline", "target"]),
                font_size=f"{random.randint(20, 40)}sp",
                pos_hint={
                    "center_x": random.random(),
                    "center_y": random.random()
